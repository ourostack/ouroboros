...
#define REQUIRED_FIELDS 2U
#define PLAN_FIELDS 9U
#define MAXIMUM_ASSETS 16U
#define MAXIMUM_FIELD_BYTES 1048576U
#define MAXIMUM_FRAME_BYTES (4U + REQUIRED_FIELDS * (4U + MAXIMUM_FIELD_BYTES))
#define PLAN_PATH "developer-id-signing-native-plan.v1.bin"
...
static int safe_json_string(const byte_field *field) {
  for (size_t index = 0U; index < field->length; index += 1U) {
    const unsigned char value = field->bytes[index];
    if (value < 0x20U || value > 0x7eU || value == (unsigned char)'"'
      || value == (unsigned char)'\\') return 0;
  }
  return field->length > 0U;
}

static int safe_relative_path(const byte_field *field) {
  if (field->length == 0U || field->bytes[0] == (unsigned char)'/'
    || field->bytes[field->length - 1U] == (unsigned char)'/') return 0;
  for (size_t index = 0U; index < field->length; index += 1U) {
    const unsigned char value = field->bytes[index];
    const int allowed = (value >= (unsigned char)'A' && value <= (unsigned char)'Z')
      || (value >= (unsigned char)'a' && value <= (unsigned char)'z')
      || (value >= (unsigned char)'0' && value <= (unsigned char)'9')
  ...
  for (size_t index = 0U; index + 1U < field->length; index += 1U) {
    if (field->bytes[index] == (unsigned char)'.' && field->bytes[index + 1U] == (unsigned char)'.'
      && (index == 0U || field->bytes[index - 1U] == (unsigned char)'/')
      && (index + 2U == field->length || field->bytes[index + 2U] == (unsigned char)'/')) return 0;
  }
  return 1;
}
...
static void initialize_assets(signing_asset *assets) {
  memset(assets, 0, sizeof(*assets) * MAXIMUM_ASSETS);
  for (size_t index = 0U; index < MAXIMUM_ASSETS; index += 1U) assets[index].descriptor = -1;
}

static void clear_assets(signing_asset *assets, size_t count) {
  for (size_t index = 0U; index < count; index += 1U) {
    if (assets[index].path != NULL) {
      clear_bytes(assets[index].path, strlen(assets[index].path));
      free(assets[index].path);
    }
    if (assets[index].identifier != NULL) {
      clear_bytes(assets[index].identifier, strlen(assets[index].identifier));
      free(assets[index].identifier);
    }
    if (assets[index].descriptor >= 0) close(assets[index].descriptor);
    clear_bytes(assets[index].sha256, sizeof(assets[index].sha256));
  }
}

static int parse_assets(const byte_field *encoded, signing_asset *assets, size_t *asset_count) {
  if (encoded->length < 4U) return 0;
  const uint32_t field_count = read_u32_be(encoded->bytes);
  if (field_count < 2U || field_count > MAXIMUM_ASSETS * 2U || field_count % 2U != 0U) return 0;
  byte_field fields[MAXIMUM_ASSETS * 2U];
  memset(fields, 0, sizeof(fields));
  if (!parse_frame(encoded->bytes, encoded->length, field_count, fields)) return 0;
  const size_t count = (size_t)field_count / 2U;
  for (size_t index = 0U; index < count; index += 1U) {
    const byte_field *path = &fields[index * 2U];
    const byte_field *identifier = &fields[index * 2U + 1U];
    if (!safe_relative_path(path) || !safe_token(identifier, 0)
      || !copy_field_string(path, &assets[index].path)
      || !copy_field_string(identifier, &assets[index].identifier)) {
      clear_assets(assets, count);
      return 0;
    }
    if (index > 0U && strcmp(assets[index - 1U].path, assets[index].path) >= 0) {
      clear_assets(assets, count);
      return 0;
    }
    assets[index].descriptor = driver_open_descriptor(open(assets[index].path, O_RDONLY | O_NOFOLLOW));
    if (assets[index].descriptor < 0
      || !path_matches_descriptor(assets[index].path, assets[index].descriptor)) {
      clear_assets(assets, count);
      return 0;
    }
  }
...
static int sign_and_verify_asset(SecIdentityRef identity, signing_asset *asset) {
  if (!path_matches_descriptor(asset->path, asset->descriptor)) return 0;
  CFURLRef path = (CFURLRef)driver_cf_pointer(CFURLCreateFromFileSystemRepresentation(
    NULL,
    (const UInt8 *)asset->path,
    (CFIndex)strlen(asset->path),
    false
  ));
  SecStaticCodeRef code = NULL;
  SecCodeSignerRef signer = NULL;
  CFStringRef identifier = (CFStringRef)driver_cf_pointer(
    CFStringCreateWithCString(NULL, asset->identifier, kCFStringEncodingUTF8)
  );
  ...
  if (success) {
    signed_descriptor = driver_open_descriptor(open(asset->path, O_RDONLY | O_NOFOLLOW));
    success = signed_descriptor >= 0 && path_matches_descriptor(asset->path, signed_descriptor);
  }
  if (success) {
    close(asset->descriptor);
    asset->descriptor = signed_descriptor;
...
static int execute_signing(void) {
  int failure_stage = 1;
  ...
  signing_asset assets[MAXIMUM_ASSETS];
  initialize_assets(assets);
  size_t asset_count = 0U;
  if (!safe_token(&plan.fields[6], 0) || !safe_json_string(&plan.fields[7])
    || !parse_assets(&plan.fields[8], assets, &asset_count)) {
    clear_plan(&plan);
    fputs("invalid signing plan\n", stderr);
    return 65;
  }
  ...
  if (!read_stdin_frame(&frame, &frame_length, fields)) {
    clear_assets(assets, asset_count);
    clear_plan(&plan);
  ...
  char keychain_path[] = "/tmp/ouro-signing-XXXXXX";
  ...
  if (success) success = copy_field_string(&fields[1], &password)
    && copy_field_string(&plan.fields[6], &team)
    && copy_field_string(&plan.fields[7], &common_name);
...
#pragma clang diagnostic pop
  ...
  if (success) {
    success = driver_status(SecCertificateCopyCommonName(certificate, &imported_common_name))
      == errSecSuccess;
    if (success && cf_string_equals_bytes(imported_common_name, &plan.fields[7])) failure_stage = 81;
    else success = 0;
    imported_team = certificate_organizational_unit(certificate);
    if (success && cf_string_equals_bytes(imported_team, &plan.fields[6])) failure_stage = 82;
    else success = 0;
  }
...
#pragma clang diagnostic pop
  ...
  for (size_t index = 0U; success && index < asset_count; index += 1U) {
    success = sign_and_verify_asset(identity, &assets[index])
      && sha256_descriptor(assets[index].descriptor, assets[index].sha256)
      && path_matches_descriptor(assets[index].path, assets[index].descriptor);
  }
...
#pragma clang diagnostic pop
  ...
  if (!success) {
    ...
    clear_assets(assets, asset_count);
    fprintf(stderr, "asset signing failed at stage %d\n", failure_stage);
    return 65;
  }
  printf(
    "{\"applicationCommonName\":\"%s\",\"assets\":[",
    common_name
  );
  for (size_t index = 0U; index < asset_count; index += 1U) {
    printf(
      "%s{\"path\":\"%s\",\"sha256\":\"%s\",\"signingIdentifier\":\"%s\"}",
      index == 0U ? "" : ",",
      assets[index].path,
      assets[index].sha256,
      assets[index].identifier
    );
  }
  printf(
    "],\"certificateDerSha256\":\"%s\",\"ephemeralKeychainRemoved\":true,"
    "\"schemaVersion\":1,\"secretValuesPersisted\":false,\"teamIdentifier\":\"%s\"}\n",
    certificate_sha256,
    team
  );
  clear_bytes(common_name, strlen(common_name));
...
static int run_coverage_probes(void) {
  ...
  for (size_t index = 0U; index < sizeof(token_bytes) - 1U; index += 1U) {
    ...
    (void)safe_json_string(&token);
  }
  token.bytes = token_bytes + sizeof(token_bytes) - 1U;
  char *copy = NULL;
  (void)copy_field_string(&token, &copy);

  unsigned char empty_path_bytes[] = { 0U };
  byte_field path = { empty_path_bytes, 0U };
  (void)safe_relative_path(&path);
  unsigned char absolute_path_bytes[] = "/asset";
  path.bytes = absolute_path_bytes;
  path.length = sizeof(absolute_path_bytes) - 1U;
  (void)safe_relative_path(&path);
  unsigned char trailing_path_bytes[] = "asset/";
  path.bytes = trailing_path_bytes;
  path.length = sizeof(trailing_path_bytes) - 1U;
  (void)safe_relative_path(&path);
  unsigned char traversal_path_bytes[] = "asset/../other";
  path.bytes = traversal_path_bytes;
  path.length = sizeof(traversal_path_bytes) - 1U;
  (void)safe_relative_path(&path);
  unsigned char bad_path_bytes[] = "asset bad";
  path.bytes = bad_path_bytes;
  path.length = sizeof(bad_path_bytes) - 1U;
  (void)safe_relative_path(&path);
  unsigned char dot_path_bytes[] = ".a";
  path.bytes = dot_path_bytes;
  path.length = sizeof(dot_path_bytes) - 1U;
  (void)safe_relative_path(&path);
  unsigned char embedded_dot_path_bytes[] = "a..b";
  path.bytes = embedded_dot_path_bytes;
  path.length = sizeof(embedded_dot_path_bytes) - 1U;
  (void)safe_relative_path(&path);
  unsigned char terminal_traversal_path_bytes[] = "..";
  path.bytes = terminal_traversal_path_bytes;
  path.length = sizeof(terminal_traversal_path_bytes) - 1U;
  (void)safe_relative_path(&path);
  unsigned char prefixed_traversal_path_bytes[] = "..a";
  path.bytes = prefixed_traversal_path_bytes;
  path.length = sizeof(prefixed_traversal_path_bytes) - 1U;
  (void)safe_relative_path(&path);
  unsigned char high_ascii_path_bytes[] = "a{";
  path.bytes = high_ascii_path_bytes;
  path.length = sizeof(high_ascii_path_bytes) - 1U;
  (void)safe_relative_path(&path);
  unsigned char path_character_bytes[] = "Aa0-_./";
  for (size_t index = 0U; index < sizeof(path_character_bytes) - 1U; index += 1U) {
    path.bytes = path_character_bytes + index;
    path.length = 1U;
    (void)safe_relative_path(&path);
  }
  ...
  initialize_assets(assets);
  size_t asset_count = 0U;
  (void)parse_assets(&empty, assets, &asset_count);
  unsigned char asset_header[] = { 0U, 0U, 0U, 34U };
  byte_field invalid_assets = { asset_header, sizeof(asset_header) };
  (void)parse_assets(&invalid_assets, assets, &asset_count);
  asset_header[3] = 3U;
  (void)parse_assets(&invalid_assets, assets, &asset_count);
  unsigned char truncated_assets[] = { 0U, 0U, 0U, 2U };
  invalid_assets.bytes = truncated_assets;
  invalid_assets.length = sizeof(truncated_assets);
  (void)parse_assets(&invalid_assets, assets, &asset_count);
  unsigned char invalid_identifier_assets[] = {
    0U, 0U, 0U, 2U,
    0U, 0U, 0U, 1U, 'a',
    0U, 0U, 0U, 1U, ' '
  };
  invalid_assets.bytes = invalid_identifier_assets;
  invalid_assets.length = sizeof(invalid_identifier_assets);
  (void)parse_assets(&invalid_assets, assets, &asset_count);
  return 0;
}
#endif
...
int main(int argc, char **argv) {
  ...
  if (argc == 2 && strcmp(argv[1], "--execute") == 0) return execute_signing();
  fputs("usage: developer-id-signing-driver --contract | --execute\n", stderr);
  return 64;
}
