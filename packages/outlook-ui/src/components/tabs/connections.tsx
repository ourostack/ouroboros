...
import type { OutlookAttentionView, OutlookBridgeInventory, OutlookFriendView } from "../../contracts"
...
export function ConnectionsTab({ agentName, focus, onFocusConsumed, refreshGeneration }: { agentName: string; focus?: string; onFocusConsumed?: () => void; refreshGeneration: number }) {
  ...
  const [attention, setAttention] = useState<OutlookAttentionView | null>(null)
  const [bridges, setBridges] = useState<OutlookBridgeInventory | null>(null)
  const [friends, setFriends] = useState<OutlookFriendView | null>(null)

  useEffect(() => {
    const base = `/agents/${encodeURIComponent(agentName)}`
    Promise.all([
      fetchJson<OutlookAttentionView>(`${base}/attention`).then(setAttention),
      fetchJson<OutlookBridgeInventory>(`${base}/bridges`).then(setBridges),
      fetchJson<OutlookFriendView>(`${base}/friends`).then(setFriends),
    ])
