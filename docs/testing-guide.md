...
`npx ouro.bot@alpha` -> `ouro up` -> `ouro status` -> `ouro chat` / daemon senses -> `ouro stop`
...
- `npx ouro.bot@alpha -v` and `ouro -v` report the same version.
...
### `ouro` and `npx ouro.bot@alpha` disagree on version
