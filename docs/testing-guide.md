...
Runtime tests for the OpenAI Realtime/SIP sessions should drive the actual production session classes (`TwilioOpenAIRealtimeMediaStreamSession` and `OpenAISipPhoneSession`) through fixture WebSocket harnesses — never a duplicate test adapter. The bottleneck invariant — that **no `response.create` reaches the OpenAI WebSocket unless the `VoiceFloorController` decision is `allow`** — must be provable by triggering each legacy followup path (user-turn-final timer, tool-completion followup, conflict-backoff retry, holding-phrase timer) while the floor model says the caller has the floor or the call is hanging up, then asserting zero `response.create` frames on the wire. No live human calls. The companion `senses.voice_floor_transition`, `senses.voice_floor_decision`, and `senses.voice_floor_gate_blocked` events make the runtime's gate behavior observable without coupling tests to the controller's internals.
