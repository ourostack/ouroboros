...
This is the harness-facing recovery guide for Agent Mail. Use it when an agent needs to diagnose native mail, delegated human-mail sources, outbound delivery state, or autonomy policy from the local runtime side.
...
| --- | --- | --- | --- |
| mail silently stopped arriving | Run `ouro doctor --category Mailroom` and read the `mail ingest liveness` check, not the `mailroom` check next to it. The `mailroom` check only proves a mailbox is configured, and its message count is cumulative so it can never fall — a configured-but-dead pipe passes it forever. `mail ingest liveness` reports the age of the newest ingest and fails past 72h (warn past 24h; override per agent with `senses.mail.freshness.{warnAfterHours,failAfterHours}` in `agent.json`). A stale result almost always means the mailbox keyIds/grant no longer match the vault after a server-side rotation — follow the "hosted registry/vault key drift" row next. | Vault unlock, or provider help when the rotation must be reversed rather than re-issued. | Age since last ingest, last-ingest timestamp, mailbox/source key ids, no private keys. |
...
| hosted authority and local cache diverge | Run `ouro doctor --category Mailroom`. The hosted index authority check uses read-only authority observation and compares complete hosted IDs/metadata with the reconstructible local search cache, explicit plaintext/encrypted provenance, current decryption keys, and durable per-message missing-key receipts. If authority is sound and doctor reports only cache divergence, run `ouro mail sync-cache --agent <agent>`; it requires consecutive equal authority fingerprints within three passes, bounds body work to 20 workers, prints progress/heartbeat output, converges local cache files atomically, and does not mutate hosted mail. If authority is transient, malformed, duplicate, or unavailable, preserve local state and repair that fault first. | Vault unlock or provider help only when fresh credentials/hosted authority cannot be resolved. | Canonical counts/fingerprint, cache counts, message IDs and key IDs only; no bodies or key material. |
...
| wrong mailbox provenance | Compare recipient, `mailboxRole`, `compartmentKind`, `ownerEmail`, and `source` in `mail_recent`, `mail_thread`, `mail_access_log`, and Ouro Mailbox. Stop if delegated human mail starts appearing as the agent's native correspondence. | Human confirms ambiguous owner/source grants. | Message id, recipient, mailbox role, source label, owner email. |
...
- `ouro mail sync-cache --agent <agent>`
- `ouro status`
- `ouro doctor`
- `mail_status`
- `mail_recent`
