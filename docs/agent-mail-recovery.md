...
| hosted authority and local cache diverge | Run `ouro doctor --category Mailroom`. The hosted index authority check uses read-only authority observation and compares complete hosted IDs/metadata with the reconstructible local search cache and current decryption-key provenance. If authority is sound and doctor reports only cache divergence, run `ouro mail sync-cache --agent <agent>`; it converges local cache files atomically and does not mutate hosted mail. If authority is transient, malformed, duplicate, or unavailable, preserve local state and repair that fault first. | Vault unlock or provider help only when fresh credentials/hosted authority cannot be resolved. | Canonical counts/fingerprint, cache counts, message IDs and key IDs only; no bodies or key material. |
