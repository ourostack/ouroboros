...
- `openai-compatible` uses `providers/openai-compatible`, the canonical base URL `https://api.z.ai/api/paas/v4/`, and GLM models (`glm-5.3-flash` by default; `glm-4.5-flash` is an explicit zero-cost bootstrap probe; `glm-4.7-flash` remains an explicit same-lane probe; `glm-5.2` and `glm-5-turbo` remain explicit same-lane alternatives).
...
`ouro connect bluebubbles` is the standard setup for this machine's BlueBubbles attachment. It prompts for the local server URL, app password, webhook listener settings, stores them in `runtime/machines/<machine-id>/config`, enables `senses.bluebubbles.enabled` in `agent.json`, and sets up host continuity. For a same-user bridge, Ouro installs or verifies the native-compatible `com.bluebubbles.server` LaunchAgent. For a bridge in another logged-in macOS account, Ouro installs a generic helper and returns one nonce-bound `human-required` Terminal command plus a matching `ouro bluebubbles host collect --request-id <id>` command; it never requests, stores, or logs that account's login password.

### Sanctuary Jellyfin catalog access

The Sanctuary media-optimization read uses one dedicated Jellyfin user, not an administrator API key and not a Jellyfin database scrape. In Jellyfin's Users dashboard, create a user for Mendelow Cloud Butler, leave administrator access off, disable access to all libraries, and grant exactly the Movies and TV libraries. Disable all devices and allow only the device id `mendelow-cloud-butler-sanctuary`. Keep playback, transcoding, remuxing, conversion, remote access/control, live TV, public sharing, deletion, download, and management permissions off. Signing in or issuing a token is `human-required` because it can involve the Jellyfin dashboard and secret entry.

Store the dedicated user's ID, exact two library IDs, and access token in Sanctuary's existing machine-scoped vault config from inside the canonical Butler container. The nonsecret IDs may use `--value`; omit `--value` for the token so the CLI reads it through its hidden prompt instead of command history:

```sh
docker exec -it ouro-butler node /opt/ouro/dist/heart/daemon/ouro-entry.js vault config set --agent sanctuary --scope machine --key jellyfin.userId --value '<user-id>'
docker exec -it ouro-butler node /opt/ouro/dist/heart/daemon/ouro-entry.js vault config set --agent sanctuary --scope machine --key jellyfin.folderIds --value '<movies-folder-id>,<tv-folder-id>'
docker exec -it ouro-butler node /opt/ouro/dist/heart/daemon/ouro-entry.js vault config set --agent sanctuary --scope machine --key jellyfin.accessToken
```

Run `docker exec -it ouro-butler node /opt/ouro/dist/heart/daemon/ouro-entry.js vault config status --agent sanctuary --scope machine` and verify that it reports the field names `jellyfin.userId`, `jellyfin.folderIds`, and `jellyfin.accessToken` without printing their values, then use the existing `ouro-butler` container/runtime refresh or restart path. Do not run a separate host-native `ouro up`. Ask the butler to inspect media storage after the container is healthy. A successful read verifies live that the configured Jellyfin identity is enabled, non-administrative, restricted to the exact configured libraries and Butler device, and unable to use the disabled capabilities before reading the catalog. Jellyfin uses only fixed GET endpoints and omits `userId` from `/Items` so the token's own scope applies. If the optional `jellyfin` attachment is absent, Sanctuary still starts and the read returns Unmanic evidence with Jellyfin marked unavailable. Unmanic uses fixed read endpoints plus its one read-shaped `POST /unmanic/api/v2/pending/tasks`, with a fixed bounded body. The result labels every upstream string as untrusted metadata, tells the agent never to follow instructions embedded in it, and never returns tokens, user IDs, item IDs, source IDs, or filesystem paths.

To rotate the credential, issue a replacement token for the same restricted user, replace `jellyfin.accessToken` through the hidden container vault prompt, refresh the existing container runtime, verify the media read, and then revoke the old token. For immediate rollback, disable or delete the dedicated Jellyfin user and revoke its token; the Butler will retain Unmanic evidence and report Jellyfin as unavailable. Never put either token in a committed file, shell argument, log, or chat message.

After the listener binds, connect reconciles exactly one machine-and-agent-owned BlueBubbles `[*]` webhook. The running sense repeats the bounded, single-flight reconciliation every 180 seconds, preserves unrelated callbacks, and creates the desired callback before deleting a stale owned one. If the listener or BlueBubbles API is unavailable, the connector reports saved-but-incomplete instead of claiming success. `ouro doctor` reports whether the owned webhook is exact, missing, drifted, API-unreachable, or auth-failed without printing the secret-bearing callback URL. `ouro bluebubbles host status --json` reports app, plist/service, process, and HTTP health separately; cross-user launchd receipts are point-in-time evidence, not current serving proof.

An exact webhook plus a healthy upstream proves configuration and reachability, not that a person recently wrote. With no recent inbound event, doctor reports quiet/unverified and does not synthesize an inbound message. Repair actual host or webhook faults through the standard `ouro connect bluebubbles --agent <agent>` path; do not create a fake message merely to turn quiet evidence green.

If Ouro is already running, the connector recycles the daemon once so the local listener starts with the freshly saved machine attachment; if Ouro is not running, the next `ouro up` loads it. The attachment and host setup are machine-local and do not make that Mac Messages bridge portable to every machine.
...
Ouro Workbench is a local machine sense for terminal/TUI agents, but it is not enabled by a synced `agent.json` entry. When the native Workbench app launches a boss agent, it starts `ouro acp-serve --agent <agent> --workbench-mcp`, which injects the `ouro_workbench` tools for that served turn without writing `senses.workbench.enabled` or `mcpServers.ouro_workbench` into the bundle. The native app supplies the exact Bun executable, bundled cmux CLI, live control socket, and inherited cmux socket capability as turn-local MCP environment; the daemon forwards only those four values to the MCP child and does not journal them. The authoritative active signal is the presence of `workbench_*` tools in that turn. `ouro connect workbench --agent <agent>` verifies the installed `OuroWorkbenchMCP` binary and removes stale Workbench bundle entries left by old setups; it does not persist a Workbench server or sense. Workbench setup does not require provider secrets beyond the selected boss agent's existing vault-backed provider lanes; browser login, MFA, raw secret entry, and Apple Developer notarization remain human-required when they are actually needed.
...
ouro vault config set --agent <agent> --key voice.twilioOutboundConversationEngine --value openai-realtime --scope machine
