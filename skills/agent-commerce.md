...
4. Human approves in a new message that includes `CONFIRM_PURCHASE`, the checkout id, and the preview digest
...
Money-moving tools (`stripe_create_card`, `flight_hold`, `flight_book`) require a `commerce_authority` token from `commerce_checkout_commit`. This is the local AP2-compatible primitive: an exact mandate record with merchant, amount, currency, allowed tool, exact tool constraints, reason, digest, expiry, confirmation, and access log. If the tool, amount, currency, offer id, card type, or other constraint changes, create a new preview and get a new confirmation.
