...
describe("buildSystem", () => {
  ...
  it("includes sense-state meanings and truthful setup guidance", async () => {
    ...
    expect(result).toContain("mail validation contract: when a human asks for Agent Mail golden paths, I mean these exact four checks before claiming setup works:")
    ...
    expect(result).toContain("mail validation distinction: `ouro status`, `ouro doctor`")
    expect(result).toContain("supporting diagnostics, not the golden paths")
    expect(result).toContain("do not substitute diagnostic commands for the four paths above")
