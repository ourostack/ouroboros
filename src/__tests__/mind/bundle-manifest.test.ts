...
describe("bundle-manifest", () => {
  ...
  it("includes the context-loss Sentinel flight-recorder directory in the canonical manifest", () => {
    ...
    expect(paths).toContain("arc/flight-recorder/context-loss-sentinel/history")
    expect(paths).toContain("arc/flight-recorder/context-loss-sentinel/receipts")
    const entry = CANONICAL_BUNDLE_MANIFEST.find((item) => item.path === "arc/flight-recorder/context-loss-sentinel")
    expect(entry?.kind).toBe("dir")
    expect(CANONICAL_BUNDLE_MANIFEST.find((item) => item.path === "arc/flight-recorder/context-loss-sentinel/history")?.kind)
      .toBe("dir")
    expect(CANONICAL_BUNDLE_MANIFEST.find((item) => item.path === "arc/flight-recorder/context-loss-sentinel/receipts")?.kind)
      .toBe("dir")
  })
