...
describe("package metadata", () => {
  ...
  it("keeps both root lockfile executable maps aligned with the published package", () => {
    for (const name of ["package-lock.json", "npm-shrinkwrap.json"]) {
      ...
      expect(lock.packages[""].bin).toEqual(packageJson.bin)
    }
  })
  ...
  it("gates publish on both packed artifact E2E suites", () => {
    ...
    expect(packageJob).toContain('chmod 0700 "$AUDIT_PERMISSION_ROOT"')
    expect(packageJob).toContain('chmod 0600 "$AUDIT_PERMISSION_ROOT"/*.json')
    expect(packageJob).toContain('sudo chown root:root "$AUDIT_PERMISSION_ROOT"/*')
    expect(packageJob).toContain('if docker run --rm --network none')
    expect(packageJob).toContain('--user 0:0 --read-only')
    expect(packageJob).toContain('--entrypoint /opt/ouro/deploy/unraid/audit-container-spec.sh')
