...
describe("Mendelow Cloud Butler Community Apps release", () => {
  ...
  it("installs privileged event assets from the exact reviewed image before Butler mutation", () => {
    ...
    expect(repositoryStageIndex).toBeGreaterThan(policyExtractIndex)
    expect(stagedAuditIndex).toBeGreaterThan(repositoryStageIndex)
    expect(installIndex).toBeGreaterThan(stagedAuditIndex)
  ...
  it("proves Community Apps recognition through the active inline or delegated implementation", () => {
    const runbook = fs.readFileSync("deploy/unraid/README.txt", "utf8")

    expect(runbook).toContain('$communityAppsEntryPath = "/usr/local/emhttp/plugins/community.applications/include/exec.php";')
    expect(runbook).toContain('$communityAppsHelperPath = "/usr/local/emhttp/plugins/community.applications/include/previous_apps_helpers.php";')
    expect(runbook).toContain('$communityAppsDependencies = ["/usr/local/emhttp/plugins/community.applications/include/paths.php", "/usr/local/emhttp/plugins/community.applications/include/plugin_identity.php", "/usr/local/emhttp/plugins/community.applications/include/helpers.php"];')
    expect(runbook).toContain("php_strip_whitespace($path)")
    expect(runbook).toContain("(fileperms($path) & 0022) !== 0")
    expect(runbook).toContain('proc_open(["/usr/bin/php", "-l", $path]')
    expect(runbook).toContain("$entrySource = $readActivePhp($communityAppsEntryPath)")
    expect(runbook).toContain('hash("sha256", $inlineBody) === "1914b9c6b8910b7bb116dad48fe76d4790a069becbf0d68b3822ee3df7f22b1c"')
    expect(runbook).toContain('hash("sha256", $delegatedBody) === "16b52cd87cc3f74915bf172f8a6eb46f981529208766916d4aeedf4b3344a3a4"')
    expect(runbook).toContain('"previous-apps-inline-v1"')
    expect(runbook).toContain('"previous-apps-helper-v1"')
    expect(runbook).toContain('"PreviousAppsHelpers::collectDockerApplications"')
    expect(runbook).toContain("PreviousAppsHelpers::collectDockerApplications(true, \"true\", \"docker\"")
    expect(runbook).toContain("foreach ($communityAppsDependencies as $dependencyPath) $readActivePhp($dependencyPath);")
    expect(runbook).toContain('"entryPath" => $communityAppsEntryPath')
    expect(runbook).toContain('"entryFunction" => "previous_apps"')
    expect(runbook).toContain('"implementationPath" => $communityAppsImplementationPath')
    expect(runbook).toContain('"implementationSymbol" => $communityAppsImplementationSymbol')
    expect(runbook).not.toContain("token_get_all")
    expect(runbook).not.toContain("$containsEvery")
    expect(runbook).not.toContain('$communityAppsSource = file_get_contents($communityAppsSourcePath)')
    const programStart = runbook.indexOf("timeout -s KILL 20 /usr/bin/php -r '\n", runbook.indexOf("verify_dockerman_and_community_apps()"))
    const programEnd = runbook.indexOf("\n      ' \"$VERIFY_VERSION_IMAGE\"", programStart)
    const program = runbook.slice(programStart, programEnd)
    expect(programStart).toBeGreaterThan(-1)
    expect(programEnd).toBeGreaterThan(programStart)
    expect(program.match(/'/gu)).toHaveLength(1)
  })
  ...
  it("audits and installs original version-tagged bytes separately from the transient exact-ID copy", () => {
    ...
    expect(runbook.match(/inspect_registry_manifest_digest "\$VERSION_IMAGE"/gu)).toHaveLength(2)
    expect(update.match(/inspect_registry_manifest_digest "\$VERSION_IMAGE"/gu)).toHaveLength(2)
