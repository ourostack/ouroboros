...
describe("Sanctuary Unit 16 host broker", () => {
  ...
  it("keeps host authority fixed and emits only redacted broker failures", () => {
    ...
    expect(source).toContain('{ destination: "/run/ouro-events", source: PRODUCTION_EVENT_SPOOL_SOURCE, propagation: "rprivate", rw: false, type: "bind" }')
    expect(source).not.toContain("/run/sanctuary/sabnzbd.ini")
    expect(source).toContain("mount.source === expected.source")
    expect(source).not.toContain("mount.mode === expected.mode")
    expect(source).toContain("mode: mount.Mode")
    expect(source).toContain("mount.propagation === expected.propagation")
    expect(source).toContain('"capDrop":{{json .HostConfig.CapDrop}}')
    expect(source).toContain("value.capDrop === null || (Array.isArray(value.capDrop) && value.capDrop.length === 0)")
    expect(source).toContain("value.securityOpt === null || (Array.isArray(value.securityOpt) && value.securityOpt.length === 0)")
