...
import {
  ...
  readNotesView,
  readNeedsMeView,
} from "../../../heart/mailbox/readers/runtime-readers"
import {
  readChangesView,
  ...
  readOrientationView,
  readSelfFixView,
} from "../../../heart/mailbox/readers/continuity-readers"
...
function makeBundleRoot(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "mailbox-read-"))
}

describe("mailbox direct reads", () => {
  afterEach(() => {
    vi.restoreAllMocks()
  })

  it("reads machine and per-agent Mailbox state directly from existing bundle truth", async () => {
    ...
    const alpha = readMailboxAgentState("alpha", {
      bundlesRoot,
      now: () => new Date("2026-03-29T12:00:00.000Z"),
    })
  ...
  it("re-reads request-time truth instead of holding hidden dashboard state", async () => {
    ...
    const first = readMailboxAgentState("alpha", {
      bundlesRoot,
      now: () => new Date("2026-03-29T12:00:00.000Z"),
    })
    ...
    const second = readMailboxAgentState("alpha", {
      bundlesRoot,
      now: () => new Date("2026-03-29T12:00:00.000Z"),
    })
  ...
  it("handles sparse or malformed direct-read inputs without inventing state", async () => {
    ...
    const { readMailboxAgentState, readMailboxMachineState } = await import("../../../heart/mailbox/mailbox-read")

    const gamma = readMailboxAgentState("gamma", {
      bundlesRoot,
      now: () => new Date("2026-03-29T12:00:00.000Z"),
    })
    ...
    const delta = readMailboxAgentState("delta", {
      bundlesRoot,
      now: () => new Date("2026-03-29T12:00:00.000Z"),
    })
    ...
    const epsilon = readMailboxAgentState("epsilon", {
      bundlesRoot,
      now: () => new Date("2026-03-29T12:00:00.000Z"),
    })
    expect(epsilon.coding.items).toEqual([])

    const zeta = readMailboxAgentState("zeta", {
      bundlesRoot,
      now: () => new Date("2026-03-29T12:00:00.000Z"),
    })
    ...
    const theta = readMailboxAgentState("theta", {
      bundlesRoot,
      now: () => new Date("2026-03-29T12:00:00.000Z"),
    })
  ...
  it("records non-Error coding read failures truthfully", async () => {
    ...
    const { readMailboxAgentState } = await import("../../../heart/mailbox/mailbox-read")
    const eta = readMailboxAgentState("eta", {
      bundlesRoot,
      now: () => new Date("2026-03-29T12:00:00.000Z"),
    })
    ...
    const iota = readMailboxAgentState("iota", {
      bundlesRoot,
      now: () => new Date("2026-03-29T12:00:00.000Z"),
    })
  ...
  it("uses default discovery and runtime dependencies when options are omitted", async () => {
    ...
    const agent = readMailboxAgentState("alpha")
    const machine = readMailboxMachineState()
...
describe("mailbox deep readers", () => {
  ...
  describe("readDeskPrefs", () => {
    ...
    it("reads and filters structured desk prefs", async () => {
      const tmpRoot = makeBundleRoot()
      const agentRoot = path.join(tmpRoot, "agent.ouro")
      writeJson(path.join(agentRoot, "state", "mailbox-prefs.json"), {
    ...
    it("reads legacy Outlook desk prefs during the Mailbox rename", async () => {
      const tmpRoot = makeBundleRoot()
      const agentRoot = path.join(tmpRoot, "agent.ouro")
      writeJson(path.join(agentRoot, "state", "outlook-prefs.json"), {
        carrying: "legacy carrying block",
        dismissedObligations: ["ob-legacy"],
      })

      const prefs = readDeskPrefs(agentRoot)

      expect(prefs.carrying).toBe("legacy carrying block")
      expect(prefs.dismissedObligations).toEqual(["ob-legacy"])
    })
  ...
  describe("readNeedsMeView", () => {
    it("aggregates reply, obligation, pending, and habit work while respecting desk prefs", async () => {
      ...
      writeJson(path.join(alphaRoot, "friends", "friend-1.json"), { name: "Ari" })
      writeJson(path.join(alphaRoot, "friends", "friend-2.json"), { name: "Sam" })
      writeJson(path.join(alphaRoot, "state", "mailbox-prefs.json"), {
  ...
  describe("readChangesView", () => {
    ...
    it("detects changes when obligations shift between snapshots", async () => {
      ...
      const snapshotDir = path.join(agentRoot, "state", "mailbox")
    ...
    it("reads legacy Outlook active-work snapshots during the Mailbox rename", async () => {
      agentRoot = fs.mkdtempSync(path.join(os.tmpdir(), "changes-legacy-"))
      const snapshotDir = path.join(agentRoot, "state", "outlook")
      fs.mkdirSync(snapshotDir, { recursive: true })
      fs.writeFileSync(path.join(snapshotDir, "active-work-snapshot.json"), JSON.stringify({
        obligationSnapshots: [
          { id: "ob-legacy", status: "pending", artifact: null, nextAction: null },
        ],
        codingSnapshots: [],
        timestamp: "2026-04-03T09:00:00Z",
      }), "utf-8")
      writeJson(path.join(agentRoot, "arc", "obligations", "ob-legacy.json"), {
        id: "ob-legacy",
        status: "in_progress",
        content: "Rename surface",
        origin: { friendId: "ari", channel: "cli", key: "chat" },
        createdAt: "2026-04-03T08:00:00Z",
        updatedAt: "2026-04-03T10:00:00Z",
      })
      ...
      expect(view.items.some((c) => c.id === "ob-legacy")).toBe(true)
      expect(fs.existsSync(path.join(agentRoot, "state", "mailbox", "active-work-snapshot.json"))).toBe(true)
    })
  ...
  describe("readObligationDetailView", () => {
    ...
    it("falls back to the raw obligation currentSurface when normalized summary omits that obligation", async () => {
      ...
      vi.resetModules()
      vi.doMock("../../../heart/mailbox/readers/agent-machine", async (importOriginal) => {
        const actual = await importOriginal<typeof import("../../../heart/mailbox/readers/agent-machine")>()
        return {
          ...actual,
      ...
      try {
        const { readObligationDetailView: readWithFallback } = await import("../../../heart/mailbox/readers/continuity-readers")
        ...
        vi.doUnmock("../../../heart/mailbox/readers/agent-machine")
        vi.resetModules()
      }
    })
  ...
  describe("readNoteDecisionView", () => {
    ...
    it("reads legacy Outlook note decisions during the Mailbox rename", async () => {
      const agentRoot = fs.mkdtempSync(path.join(os.tmpdir(), "legacy-note-decisions-"))
      try {
        const logPath = path.join(agentRoot, "state", "outlook", "note-decisions.jsonl")
        fs.mkdirSync(path.dirname(logPath), { recursive: true })
        fs.writeFileSync(logPath, [
          JSON.stringify({ kind: "friend", decision: "promote", timestamp: "2026-04-03T09:00:00Z", id: "legacy-1" }),
        ].join("\n") + "\n", "utf-8")

        const view = readNoteDecisionView(agentRoot)

        expect(view.totalCount).toBe(1)
        expect(view.items[0]?.id).toBe("legacy-1")
      } finally {
        fs.rmSync(agentRoot, { recursive: true, force: true })
      }
    })
