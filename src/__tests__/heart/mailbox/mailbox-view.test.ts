...
describe("mailbox machine view", () => {
  it("builds a machine-overview-first view with daemon truth, entrypoints, and attention-sorted agents", async () => {
    const { buildMailboxMachineView } = await import("../../../heart/mailbox/mailbox-view")

    const view = buildMailboxMachineView({
      machine: {
...
describe("mailbox agent view", () => {
  it("builds a read-only human-default agent view with summary-only inward work and recent activity", async () => {
    const { buildMailboxAgentView } = await import("../../../heart/mailbox/mailbox-view")

    const view = buildMailboxAgentView({
      agent: {
        productName: "Ouro Mailbox",
        agentName: "slugger",
        ...
        tasks: {
          ...
          liveTaskNames: ["Build Mailbox", "Review daemon seam"],
