...
import type { MailboxHttpServerHandle } from "../../../heart/mailbox/mailbox-http"

/**
 * Covers the OuroDaemon Mailbox HTTP server lifecycle via the injected
 * mailboxServerFactory seam. Previously this code path sat behind a v8
...
function makeStubHandle(stop: () => Promise<void>): MailboxHttpServerHandle {
  return {
...
describe("OuroDaemon mailbox server lifecycle", () => {
  afterEach(() => {
    vi.restoreAllMocks()
  })

  it("starts the injected mailbox server during daemon.start and stops it during daemon.stop", async () => {
    const socketPath = tmpSocketPath("mailbox-lifecycle-happy")
    const bundlesRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mailbox-lifecycle-"))
  ...
  it("continues daemon startup when the mailbox factory throws, emitting a warn event and leaving mailboxServer unset", async () => {
    const socketPath = tmpSocketPath("mailbox-lifecycle-throw")
    const bundlesRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mailbox-lifecycle-"))
  ...
  it("does not recreate the mailbox server on a second daemon.start call", async () => {
    const socketPath = tmpSocketPath("mailbox-lifecycle-double-start")
    const bundlesRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mailbox-lifecycle-"))
    const stopSpy = vi.fn(async () => undefined)
    const factory = vi.fn(async () => makeStubHandle(stopSpy))
