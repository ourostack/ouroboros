...
validate_sanctuary_roots "$RUNTIME_ROOT" "$AGENT_ROOT"`
  ...
  it("ships a complete exact-image host launcher for every Unit 16 harness command", () => {
    ...
    expect(launcher).toContain("TIME_LIMIT=1860")
    expect(launcher).toContain("TIME_LIMIT=660")
    ...
    expect(launcher).toContain('evidence-snapshot) TIME_LIMIT=1860; NETWORK=host; INPUT=no; BUNDLE_MODE=readonly; BROKER=yes')
    expect(launcher).toContain('\nstart_broker\nEXPECTED_CONFIG=')
    expect(launcher).toContain('dst=/run/ouro-acceptance/boot-id,readonly')
    expect(launcher).toContain('/usr/bin/docker stop --time 30 "$PRODUCTION_CONTAINER"')
    ...
    expect(launcher).toContain('/usr/bin/docker start "$PRODUCTION_CONTAINER"')
    ...
    expect(launcher.indexOf("PRODUCTION_STOPPED=yes", launcher.indexOf("quiesce_production_telegram_poller")))
      .toBeLessThan(launcher.indexOf('/usr/bin/docker stop --time 30 "$PRODUCTION_CONTAINER"'))
    ...
    expect(launcher).toContain('test "$(stat -c \'%u:%g %a\' "$ACCEPTANCE_STATE_ROOT")" = "10001:10001 700"')
    expect(launcher).toContain('src=$ACCEPTANCE_STATE_ROOT,dst=/home/ouro/AgentBundles/sanctuary.ouro/state/acceptance')
    expect(launcher.match(/src=\$ACCEPTANCE_STATE_ROOT,dst=/gu)).toHaveLength(1)
    ...
    expect(launcher).toContain('/sbin/reboot')
    ...
    expect(callbackHelper).toContain('mv -- "$UNIT16_CALLBACK_FILE" "$UNIT16_CALLBACK_QUARANTINE_FILE"')
