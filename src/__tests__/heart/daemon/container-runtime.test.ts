...
inspect_registry_manifest_digest "$VERSION_IMAGE"`
  ...
  it("retires obsolete source admission and adoption entrypoints", () => {
    const runbook = fs.readFileSync("deploy/unraid/README.txt", "utf8")
    for (const retired of [
      "prepackage-alpha797", "assert_prepackage_alpha797_source", "validate_sanctuary_legacy_staging",
      "prepare_sanctuary_legacy_adoption", "capture_sanctuary_legacy_evidence", "install_from_legacy_staging",
      "adoption-source-exact", "docker start ouro-butler-staging",
    ]) expect(runbook).not.toContain(retired)
    expect(runbook).toContain("assert_legacy_alpha742_source")
    expect(runbook).toContain('test "$IMAGE_ID" != sha256:e337dff04c92d116b269052f473b26a47eea933d017d1befc73af50dd37bb08d')
    expect(runbook).toContain('test "$AUDIT_RUNNER_IMAGE_ID" != sha256:e337dff04c92d116b269052f473b26a47eea933d017d1befc73af50dd37bb08d')
  })

  it("rejects retired or unsafe sources before the full Update prelude can mutate", () => {
    const runbook = fs.readFileSync("deploy/unraid/README.txt", "utf8")
    ...
    const legacyStaging = runbook.includes("    validate_sanctuary_legacy_staging() {")
...
docker() {
  ...
  if test "$1 $2" = "inspect --format"; then
    case "$3" in
      ...
      "{{.Id}}") command printf '%s\n' "$LEGACY_CONTAINER_ID" ;;
      "{{.Image}}") source_image ;;
      "{{.Config.Image}}") case "$SCENARIO" in canonical-alpha797|staging-alpha797) source_image ;; *) command printf '%s\n' "$PACKAGE_IMAGE_REFERENCE" ;; esac ;;
...
${updatePrelude}`
    ...
    try {
      ...
      for (const scenario of ["canonical-alpha797", "staging-alpha797", "staging-unknown-image", "canonical-unknown-image", "disallowed-name"]) {
      ...
      for (const scenario of ["canonical-alpha742", "package-managed"]) {
        fs.writeFileSync(callLog, "", { mode: 0o600 })
        const result = runConditionalHelper(script, scenario, environment)
...
if [ "$SCENARIO" = available ] || [ "$SCENARIO" = import-failure ]; then
  ...
  it("keeps explicit provider authentication separate from bootstrap and readiness", () => {
    const runbook = fs.readFileSync("deploy/unraid/README.txt", "utf8")
    const bootstrap = extractRunbookFunction(runbook, "bootstrap_sanctuary_vault")
...
validate_sanctuary_roots "$RUNTIME_ROOT" "$AGENT_ROOT" live-precutover`
  ...
  it("keeps standalone provider readiness out of normal updates", () => {
    const runbook = fs.readFileSync("deploy/unraid/README.txt", "utf8")
    const normalUpdate = runbook.slice(runbook.indexOf("Update:"), runbook.indexOf("Backup:"))
    expect(normalUpdate).toContain("For normal updates")
    expect(runbook).toContain("    verify_sanctuary_provider_readiness() {")
...
await_post_audit_health`
  ...
  it("admits only canonical sources and the remaining pinned alpha.742 exception", () => {
    const runbook = fs.readFileSync("deploy/unraid/README.txt", "utf8")
    ...
    expect(dispatch).not.toContain("assert_prepackage_alpha797_source")
    ...
    expect(audit).toContain('legacy-alpha742) set -- --mount-contract "$AUDIT_MOUNT_CONTRACT" ;;')
  ...
  it("rolls final production activation back under set -eu before propagating failure", () => {
    ...
    const cleanupFailure = activation.indexOf("PRODUCTION_ACTIVATION_CLEANUP_STATUS=1", failedRemove)
    expect(activation).not.toMatch(/(?:^|\n)\s+ROLLBACK_IMAGE_ID=\$\(docker inspect --format '\{\{\.Image\}\}' ouro-butler-rollback\)/u)
    const safeRestore = activation.indexOf('if test "$PRODUCTION_ACTIVATION_CLEANUP_STATUS" -eq 0; then', cleanupFailure)
    const currentImage = activation.indexOf("CURRENT_ROLLBACK_IMAGE_ID=$(docker inspect --format '{{.Image}}' ouro-butler-rollback)", safeRestore)
    const exactImage = activation.indexOf('test "$CURRENT_ROLLBACK_IMAGE_ID" = "$ROLLBACK_IMAGE_ID"', currentImage)
    const bundleRollback = activation.indexOf('migrate_sanctuary_package_managed_bundle "$IMAGE_ID" rollback', exactImage)
    const rename = activation.indexOf("docker rename ouro-butler-rollback ouro-butler", safeRestore)
    ...
    const journalRollback = activation.indexOf('"$STAGED_DOCKERMAN_TRANSACTION" rollback', finalizeRollback)
    const cleanupExit = activation.indexOf('if test "$PRODUCTION_ACTIVATION_CLEANUP_STATUS" -ne 0; then', journalRollback)
    const propagate = activation.indexOf('(exit "$PRODUCTION_ACTIVATION_STATUS")', cleanupExit)
    ...
    expect(cleanupFailure).toBeGreaterThan(failedRemove)
    expect(safeRestore).toBeGreaterThan(cleanupFailure)
    expect(currentImage).toBeGreaterThan(safeRestore)
    expect(exactImage).toBeGreaterThan(currentImage)
    expect(bundleRollback).toBeGreaterThan(exactImage)
    expect(rename).toBeGreaterThan(bundleRollback)
    ...
    expect(activation.indexOf("\n      fi\n", finalizeRollback)).toBeLessThan(journalRollback)
    expect(journalRollback).toBeGreaterThan(finalizeRollback)
    expect(cleanupExit).toBeGreaterThan(journalRollback)
    expect(propagate).toBeGreaterThan(cleanupExit)
  })
...
verify_known_good_rollback_artifact "$IMAGE_ID"`
  ...
  it("recovers exact current template topologies while preserving quarantined legacy evidence", () => {
    const runbook = fs.readFileSync("deploy/unraid/README.txt", "utf8")
    const recovery = extractRunbookFunction(runbook, "recover_dockerman_template_transaction")
    ...
    const rejectStaging = recovery.indexOf("! docker container inspect ouro-butler-staging >/dev/null 2>&1 || return 1", noProduction)
    ...
    expect(rejectStaging).toBeGreaterThan(noProduction)
    expect(normalRollback).toBeGreaterThan(rejectStaging)
  ...
  it("rejects retired staging recovery without restarting or changing autostart", () => {
    const runbook = fs.readFileSync("deploy/unraid/README.txt", "utf8")
    const recovery = extractRunbookFunction(runbook, "recover_dockerman_template_transaction").replaceAll("/usr/local/bin/node", "node")
    const oldImage = "sha256:e337dff04c92d116b269052f473b26a47eea933d017d1befc73af50dd37bb08d"
    const targetImage = `sha256:${"b".repeat(64)}`
    const versionImage = "ghcr.io/ourostack/ouroboros-butler:0.1.0-alpha.798"
...
start_only_butler_for_recovery`
  ...
  it("finishes safe preparation rollback before reporting failed stale-container cleanup", () => {
    const runbook = fs.readFileSync("deploy/unraid/README.txt", "utf8")
    const update = runbook.slice(runbook.indexOf("For normal updates"), runbook.indexOf("Backup:"))
    const preparationFailure = update.slice(update.indexOf("PRODUCTION_PREPARATION_STATUS=$?"), update.indexOf("Preparation failure therefore"))

    expect(preparationFailure).toContain("PRODUCTION_PREPARATION_CLEANUP_STATUS=0")
    expect(preparationFailure).toContain("PRODUCTION_PREPARATION_RECOVERY_SAFE=true")
    expect(preparationFailure).not.toContain("! docker container inspect ouro-butler-rollback >/dev/null 2>&1 || exit 1")
    const preparationCleanupFailure = preparationFailure.indexOf("PRODUCTION_PREPARATION_CLEANUP_STATUS=1")
    const preparationUnsafe = preparationFailure.indexOf("PRODUCTION_PREPARATION_RECOVERY_SAFE=false", preparationCleanupFailure)
    const preparationBundleRollback = preparationFailure.indexOf('rollback_sanctuary_bundle_if_pending "$IMAGE_ID"', preparationCleanupFailure)
    const preparationSafeRestore = preparationFailure.indexOf('if test "$PRODUCTION_PREPARATION_RECOVERY_SAFE" = true; then', preparationBundleRollback)
    const preparationFinalize = preparationFailure.indexOf('finalize_sanctuary_bundle_rollback_if_retained "$IMAGE_ID"', preparationSafeRestore)
    const preparationJournalRollback = preparationFailure.indexOf('"$STAGED_DOCKERMAN_TRANSACTION" rollback', preparationBundleRollback)
    const preparationCleanupExit = preparationFailure.indexOf('if test "$PRODUCTION_PREPARATION_CLEANUP_STATUS" -ne 0; then', preparationJournalRollback)
    expect(preparationCleanupFailure).toBeGreaterThan(-1)
    expect(preparationUnsafe).toBeGreaterThan(preparationCleanupFailure)
    expect(preparationBundleRollback).toBeGreaterThan(preparationUnsafe)
    expect(preparationSafeRestore).toBeGreaterThan(preparationBundleRollback)
    expect(preparationFinalize).toBeGreaterThan(preparationSafeRestore)
    expect(preparationFailure.indexOf("\n        fi\n", preparationFinalize)).toBeLessThan(preparationJournalRollback)
    expect(preparationJournalRollback).toBeGreaterThan(preparationFinalize)
    expect(preparationCleanupExit).toBeGreaterThan(preparationJournalRollback)
  })
