...
validate_sanctuary_roots "$RUNTIME_ROOT" "$AGENT_ROOT"`
  ...
  it("contains production stop and rollback rename preparation under set -eu", () => {
    ...
    const migration = update.indexOf("&& migrate_sanctuary_package_managed_bundle \"$IMAGE_ID\"; then", stopped)
  ...
  it("ships Mendelow Cloud Butler with the exact persistent roots and complete bootstrap bundle", () => {
    ...
    expect(updateRunbook).toContain("preserves agent.json, desiredStates, stated or unrelated routine grants, relationships, sessions, and every other state path")
