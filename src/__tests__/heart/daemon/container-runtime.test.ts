...
validate_sanctuary_roots "$RUNTIME_ROOT" "$AGENT_ROOT"`
  ...
  it("ships a complete exact-image host launcher for every Unit 16 harness command", () => {
    ...
    expect(launcher).toContain('/usr/bin/docker stop --time 30 "$TARGET_CONTAINER_ID"')
    ...
    expect(launcher).toContain('/usr/bin/docker start "$TARGET_CONTAINER_ID"')
    ...
    expect(launcher.indexOf("PRODUCTION_STOPPED=yes", launcher.indexOf("quiesce_production_telegram_poller")))
      .toBeLessThan(launcher.indexOf('/usr/bin/docker stop --time 30 "$TARGET_CONTAINER_ID"'))
    ...
    expect(stopExact.indexOf("PRODUCTION_STOPPED=yes")).toBeLessThan(stopExact.indexOf('/usr/bin/docker stop --time 30 "$TARGET_CONTAINER_ID"'))
