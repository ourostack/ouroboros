...
validate_sanctuary_roots "$RUNTIME_ROOT" "$AGENT_ROOT"`
  ...
  it("ships a complete exact-image host launcher for every Unit 16 harness command", () => {
    ...
    expect(runbook).toContain('callback-inject callback-inject.json 3<"$CALLBACK_UPDATE_FILE"')
    expect(runbook).toContain('test "$(stat -c \'%u:%g %a\' "$CALLBACK_UPDATE_FILE")" = "0:0 600"')
