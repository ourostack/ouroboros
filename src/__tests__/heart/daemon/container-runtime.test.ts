...
validate_sanctuary_roots "$RUNTIME_ROOT" "$AGENT_ROOT"`
  ...
  it("ships a complete exact-image host launcher for every Unit 16 harness command", () => {
    ...
    expect(launcher).toContain('/bin/mount --bind "$ACCEPTANCE_STATE_ROOT" "$ACCEPTANCE_PIN_ROOT"')
    expect(launcher).toContain('/bin/mount --bind "$ACCEPTANCE_PIN_ROOT" "$ACCEPTANCE_STATE_ROOT"')
    expect(launcher.indexOf("ACCEPTANCE_ALIAS_MOUNTED=yes")).toBeLessThan(launcher.indexOf('/bin/mount --bind "$ACCEPTANCE_STATE_ROOT"'))
    ...
    expect(launcher).toContain('/usr/bin/docker exec "$PRODUCTION_CONTAINER" stat -Lc')
    ...
    expect(launcher.match(/src=\$ACCEPTANCE_PIN_ROOT,dst=/gu)).toHaveLength(1)
