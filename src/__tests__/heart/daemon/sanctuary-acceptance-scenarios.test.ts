...
describe("Sanctuary live scenario capture", () => {
    ...
    for (const label of SANCTUARY_UNIT_16_EVIDENCE_LABELS) {
      ...
      if (label === "unit-15c-1-no-callback-terminalization") {
        ...
        after.events.push(
          ...
          approvalAuditEvent("telegram.approval_prompt_terminalized", 301_000, { boundAt: 1_000, expiryObservedAt: 301_000, terminalEditStartedAt: 301_000, terminalizedAt: 301_000, buttonsRemoved: true }),
        )
      ...
      if (label === "unit-16k-timeout-stale") { after.approvals = [approval("expired")]; after.interactiveDriver = timeoutStaleDriver(); after.events.push(approvalAuditEvent("senses.telegram_approval_prompt_bound", 1_000, { boundAt: 1_000 }), approvalAuditEvent("telegram.approval_prompt_terminalized", 301_000, { boundAt: 1_000, expiryObservedAt: 301_000, terminalEditStartedAt: 301_000, terminalizedAt: 301_000, buttonsRemoved: true }), event("telegram.update_dropped")) }
  ...
  it("requires a daemon-retained stale callback settlement with zero claim and mutation for unit-16k", () => {
    ...
    after.events.push(approvalAuditEvent("senses.telegram_approval_prompt_bound", 1_000, { boundAt: 1_000 }), approvalAuditEvent("telegram.approval_prompt_terminalized", 301_000, { boundAt: 1_000, expiryObservedAt: 301_000, terminalEditStartedAt: 301_000, terminalizedAt: 301_000, buttonsRemoved: true }), event("telegram.update_dropped"))
  ...
  it.each(["unit-15c-1-no-callback-terminalization", "unit-16k-timeout-stale"] as const)("bounds %s expiry observation while permitting bounded terminal edit completion", (label) => {
    const make = (expiryObservedAt: number, terminalizedAt = expiryObservedAt) => {
      ...
      after.events = [
        approvalAuditEvent("senses.telegram_approval_prompt_bound", 1_000, { boundAt: 1_000 }),
        approvalAuditEvent("telegram.approval_prompt_terminalized", terminalizedAt, { boundAt: 1_000, expiryObservedAt, terminalEditStartedAt: expiryObservedAt, terminalizedAt, buttonsRemoved: true }),
      ]
      if (label === "unit-15c-1-no-callback-terminalization") {
