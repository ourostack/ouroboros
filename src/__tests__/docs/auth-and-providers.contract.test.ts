...
describe("auth/provider documentation contract", () => {
  it("documents how to continue using an existing agent bundle", () => {
    ...
    for (const content of [authGuide, machineGuide]) {
      ...
      expect(content).toContain("ouro connect --agent <agent>")
      expect(content).toContain("ouro connect providers --agent <agent>")
      expect(content).toContain("ouro connect embeddings --agent <agent>")
      expect(content).toContain("ouro connect teams --agent <agent>")
  ...
  it("documents how to migrate old auth-style agents without legacy credential paths", () => {
    ...
    expect(authGuide).toContain("ouro connect --agent <agent>")
    expect(authGuide).toContain("ouro connect perplexity --agent <agent>")
    expect(authGuide).toContain("ouro connect embeddings --agent <agent>")
    expect(authGuide).toContain("ouro connect teams --agent <agent>")
