...
import type { SseBroadcaster } from "./outlook-http-transport"
import type { OutlookHttpReadHooks } from "./outlook-http-hooks"
import { writeJson } from "./outlook-http-response"
import {
  normalizeLegacyOutlookApiPath,
  normalizeOutlookRequestPath,
  resolveSpaDistDir,
  serveStaticFile,
} from "./outlook-http-static"
import type {
  OutlookAgentState,
  OutlookAgentView,
  OutlookMachineState,
...
export interface OutlookHttpRouteOptions {
  host: string
  getPort(): number
...
export function createOutlookHttpRequestHandler(options: OutlookHttpRouteOptions): http.RequestListener {
  ...
  return (request, response) => {
    ...
    pathname = normalizeLegacyOutlookApiPath(pathname)
...
function handleDismissObligation(
  ...
  request.on("end", () => {
    try {
      ...
      try {
        prefs = JSON.parse(fs.readFileSync(prefsPath, "utf-8")) as Record<string, unknown>
      } catch {
