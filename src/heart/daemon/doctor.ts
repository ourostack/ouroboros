...
export function checkHabitAdapters(deps: DoctorDeps): DoctorCategory {
  const projections = deps.adapterDiagnostics?.list() ?? []
  const checks: DoctorCheck[] = projections.map((projection) => ({
    id: `habit-adapter.${projection.adapter.id}.${projection.adapter.version}`,
    label: `${projection.adapter.id}@${projection.adapter.version}`,
    status: projection.status === "healthy"
      ? "pass"
    ...
    detail: [
      ...
      `evidence=${projection.evidence.map((item) => `${item.ref}#${item.sha256}`).join(",") || "none"}`,
      `blockers=${projection.blockers.map((item) => `${item.code}:${item.actor}:${item.message}`).join(",") || "none"}`,
  ...
  return { name: "Habit Adapters", checks }
}
...
const CATEGORY_CHECKERS: Array<{ name: string; fn: CategoryChecker }> = [
  ...
  { name: "Habit Adapters", fn: checkHabitAdapters },
