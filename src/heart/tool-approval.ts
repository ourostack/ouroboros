...
export interface ExecuteApprovalDecisionOptions {
  ...
  resolveApprovalPolicy?: (toolName: string, argumentsValue: JsonObject) => ReturnType<NonNullable<ToolDefinition["approvalPolicy"]>>
...
export async function executeApprovalDecision(options: ExecuteApprovalDecisionOptions): Promise<ApprovalRecord> {
  ...
  const policy = options.resolveApprovalPolicy?.(decided.toolName, validated.value.arguments)
